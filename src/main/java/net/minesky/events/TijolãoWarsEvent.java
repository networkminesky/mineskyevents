...
public class TijolãoWarsEvent {
    ...
    private static final List<String> mapas = Arrays.asList("Mapa-1", "Mapa-2");
    private static final Random random = new Random();

    public static void iniciarEvento() {
        MineSkyEvents.event = "TijolãoWars";
        EventData.setSelectedMap(selectMapa());
        SendMessages.sendMessageEvent("TijolãoWars");
    ...
    private static void encerrarEvento() {
        ...
        SendMessages.sendMessage(new TextComponent("§6§lTijolãoWars §8| §aInfelizmente o evento não teve §l4§a players para iniciar."));
    }
    ...
    private static void teleportarJogadores() {
        Bukkit.getOnlinePlayers().forEach(player -> {
            if (Utils.isPlayerInEvent(player)) {
                player.teleport(EventData.getEventArenaLocation("TijolãoWars"), PlayerTeleportEvent.TeleportCause.COMMAND);
            }
        });
    }
    ...
    private static void enviarMensagemVencedores(Player vencedor, Player[] vencedores) {
        TextComponent encerrar = new TextComponent(String.format("§6§lTijolãoWars §8| §a1º Lugar §8- §a§l%s §8| §a2º Lugar §8- §a§l%s §8| §a3º Lugar §8- §a§l%s",
    ...
    private static void distribuirPremios(Player vencedor, Player[] vencedores) {
        ...
        SendMessages.sendPlayerMessage(vencedor, new TextComponent("§6§lTijolãoWars §8| §aVocê ganhou o §lTijolãoWars §ae como prêmio você ganhou: §l$" + premio1));
        SendMessages.sendPlayerMessage(vencedores[1], new TextComponent("§6§lTijolãoWars §8| §aVocê ganhou o §lTijolãoWars §ae como prêmio você ganhou: §l$" + premio2));
        SendMessages.sendPlayerMessage(vencedores[0], new TextComponent("§6§lTijolãoWars §8| §aVocê ganhou o §lTijolãoWars §ae como prêmio você ganhou: §l$" + premio3));
