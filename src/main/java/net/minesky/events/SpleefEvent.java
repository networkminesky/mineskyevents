...
public class SpleefEvent {
    public static boolean contagemI, contagem = false;
    ...
    public static void iniciarEvento() {
        ...
        SendMessages.sendMessageEvent("Spleef");
    ...
    private static void encerrarEvento() {
        ...
        SendMessages.sendMessage(new TextComponent("§b§lSpleef §8| §aInfelizmente o evento não teve §l4§a players para iniciar."));
    }
    ...
    private static void teleportarJogadores() {
        Bukkit.getOnlinePlayers().forEach(player -> {
            if (Utils.isPlayerInEvent(player)) {
                player.teleport(Locations.spleefA, PlayerTeleportEvent.TeleportCause.COMMAND);
            }
        });
    }
        ...
        Bukkit.getOnlinePlayers().forEach(player -> {
            if (Utils.isPlayerInEvent(player)) {
                ...
                if (!EventPlayerManager.getPlayerCheck(player)) {
                    player.getInventory().addItem(EventItem.getItem("SpleefITEM"));
    ...
    private static void enviarMensagemVencedores(Player vencedor, Player[] vencedores) {
        TextComponent encerrar = new TextComponent(String.format("§b§lSpleef §8| §a1º Lugar §8- §a§l%s §8| §a2º Lugar §8- §a§l%s §8| §a3º Lugar §8- §a§l%s",
    ...
    private static void distribuirPremios(Player vencedor, Player[] vencedores) {
        ...
        if (bound2 > 0) {
            premio2 = random.nextInt(bound2) + Config.SPLEEF_MIN_2;
        } else {
            premio2 = Config.SPLEEF_MIN_2;
        }

        if (bound3 > 0) {
            premio3 = random.nextInt(bound3) + Config.SPLEEF_MIN_3;
        } else {
            premio3 = Config.SPLEEF_MIN_3;
        }
        ...
        SendMessages.sendPlayerMessage(vencedor, new TextComponent("§b§lSpleef §8| §aVocê ganhou o §lSpleef §ae como prêmio você ganhou: §l$" + premio1));
        SendMessages.sendPlayerMessage(vencedores[1], new TextComponent("§b§lSpleef §8| §aVocê ganhou o §lSpleef §ae como prêmio você ganhou: §l$" + premio2));
        SendMessages.sendPlayerMessage(vencedores[0], new TextComponent("§b§lSpleef §8| §aVocê ganhou o §lSpleef §ae como prêmio você ganhou: §l$" + premio3));
    ...
    public static void restaurar() {
        blocksbreak.forEach((location, material) -> location.getBlock().setType(material));
        ...
        Bukkit.getOnlinePlayers().forEach(player -> {
            if (player.hasPermission("mineskyevents.notify.restored")) {
                File filefor = DataManager.getFile(player.getUniqueId().toString(), "playerdata");
                FileConfiguration configfor = DataManager.getConfiguration(filefor);
                if (configfor.getBoolean("Notification")) {
                    SendMessages.sendPlayerMessage(player, new TextComponent("§b§lSpleef §8| §aA arena foi restaurada com sucesso!"));
                }
            }
        });
    }
